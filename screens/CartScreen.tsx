        ...
        try {
            if (isLoggedIn) {
                ...
                const items = [...uniqueItemIds].map(itemId => {
                    const item = totalItems.find(item => item._id === itemId);
                    return { id: item._id, name: item.name,price:item.price, quantity: groupItems[item._id].length };
                });
            ...
            <Text className="text-center text-lg text-black mt-2 " > {restaurant.name}</Text>
                <View className=" flex-row px-10 items-center  h-20 rounded-tl-3xl">
                    <Image
                        ...
                        src="https://t3.ftcdn.net/jpg/03/24/73/92/240_F_324739203_keeq8udvv0P2h1MLYJ0GLSlTBagoXS48.jpg"
                    />
                            ...
                            {Object.entries(groupItems).map(([key, items]) => {
                                ...
                                return (
                                    <View key={key} className="flex-row items-center space-x-3 py-2 px-4 bg-white rounded-3xl mx-2 mb-3 shadow-md shadow-gray">
                                        <Text className="font-bold text-black">{totalItems.filter((item: { _id: any; }) => item._id === dish._id).length}x</Text>
                                        <Image className="h-11 w-10 rounded-full" src={dish.image} />
                                        <Text className="flex-1 font-bold text-gray-700">{dish.name}</Text>
                                        <Text className="flex-1 font-semibold text-base">$ {dish.price}</Text>
                                        <TouchableOpacity onPress={() => dispatch(removeFromCart({ restaurantId: restaurantId, itemId: dish._id }))} className="p-1 rounded-full bg-red-500">
